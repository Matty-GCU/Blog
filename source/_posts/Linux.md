...
- 2-4位：  表示文件所有者的权限，即**U（User）权限**
- 5-7位：  表示文件所属组的成员的权限，即**G（Group）权限**
- 8-10位：表示文件所有者和所属组的成员之外的用户的权限，即**O（Others）权限**
- 2-10位：A（All）权限
...
![crontab配置5+1](Linux/crontab配置5+1.png)

![crontab配置中的特殊符号](Linux/crontab配置中的特殊符号.png)

![crontab配置示例](Linux/crontab配置示例.png)
...
[Linux软件包（源码包和二进制包）](http://c.biancheng.net/view/814.html)

[Linux RPM包统一命名规则](http://c.biancheng.net/view/2868.html)
...
install & verbose & hash，安装软件包，并显示详细信息和进度条
...
总结成一句话：**基于RPM，类似Maven**

![yum命令](Linux/yum命令.png)

### 4.2 克隆虚拟机

如果是在VMware里克隆虚拟机，只需要将虚拟机关机，然后选中虚拟机-“管理”-“克隆”，然后点几个下一步就可以克隆了。

由于克隆出来的虚拟机与原来的虚拟机完全一致，所以需要更改一下静态IP和主机名。

然后我就想到了能不能把我的腾讯云的轻量级应用服务器直接克隆到阿里云那边，这样就可以方便我换个云厂商继续薅羊毛了。对于这种需求，可以直接搜索“腾讯云服务器迁移阿里云”，因为阿里云会很乐意教你怎么做的。
...
[Linux系统中的文件有扩展名吗？__佚名-2022_的博客-CSDN博客](https://wangjianno1.blog.csdn.net/article/details/24205219)（Linux系统中的文件后缀名仅仅是为了方便用户的辨认，对Linux系统本身并没有意义）

Shell脚本通常以`#!/bin/sh`或`#!/bin/bash`开头，即指定Shell解析器。

[查看系统的默认shell_BANANAML的博客-CSDN博客_查看默认shell](https://blog.csdn.net/BANANAML/article/details/103179283)（在CentOS 7中，`/bin/sh`文件是指向`/bin/bash`的软链接，也就是说CentOS 7默认的Shell解析器就是Bash）
...
1. `bash或sh 脚本的相对路径或绝对路径`（不用赋予脚本+x权限）
2. 直接输入脚本的绝对路径或相对路径，执行脚本（必须具有可执行权限+x）
3. `.或source 脚本的相对路径或绝对路径`（不用赋予脚本+x权限）

![执行shell的几种方式的区别](../../../../../Users/Matty's PC/Desktop/执行shell的几种方式的区别.png)
